		...
		if($ev->checkedin == '' && $ev->event_tickets) {
			foreach($ev->event_tickets as $tix) {
				if( $tix->exurl != '' ) {
					...
					echo '<div class="event-info-link tickets ticket-text-color"><div class="link-track" data-url="'.$tix->exurl.'" data-title="Tickets" data-eid="'.$_POST['eventid'].'"><img src="img/ticket-icon-v3.png" /><p>'.$tix->label_name.'</p></div></div>';
...
*/
		...
		if($ev->checkintime) {
			//facebook row
			$fb = $ev->facebook;
			echo '<div class="event-info-link social-facebook" data-link="'.$fb->link.'" data-name="'.addslashes($fb->name).'" data-message="'.addslashes($fb->message).'" data-caption="'.addslashes($fb->caption).'" data-picture="'.addslashes($fb->picture).'" data-description="'.addslashes($fb->description).'" data-eid="'.$_POST['eventid'].'"><img src="img/facebook-icon-v3.png" /><p>Share on Facebook</p></div>';
		}
		...
		echo '<div class="event-info-link event-info-users" data-eid="'.$_POST['eventid'].'"><img src="img/users-checked-in-v3.png" /><p>'.$ev->nocheckins.'</p></div>';
