		...
		if($events) {
			...
			foreach($events as $ev) {
				...
				if($ev_count == 0 && $ev->us_img<>'' && $ev->opp_img<>'') {
					...
					if($ev->f_temp) {
						//has weather
						echo '<span class="weather"><img src="img/weather/'.$ev->icon.'">'.$ev->temp_formatted.' - '.$ev->weather_description.'</span>';
					}
					if($ev->event_tickets) {
						echo '<span class="tickets-available"'.(($ev->event_tickets_label_color!='') ? ' style="color: '.$ev->event_tickets_label_color.'"' : '').'><img class="ticket tint-image" '.(($ev->event_tickets_label_color!='') ? ' data-label-color="'.$ev->event_tickets_label_color.'"' : '').' src="img/ticket-icon-tiny.png" />'.$ev->event_tickets_label_name.'</span>';
					}
					echo '</div></div>';
				} else {
