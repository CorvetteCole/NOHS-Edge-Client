		...
		foreach($json as $j) {
			...
			foreach($j->leaders as $k=>$l) {
				...
				if($l->title) {
					...
					if($l->current_position) {
						$html.='<li class="row-highlight captain-list-item"><div class="user-place"><span>'.$place.'</span></div>'."<img style='left: $crownPosition' class='user-pic-crown' src='img/custom/captain-crown.png'>".'<img class="user-pic user-pic-self" src="'.$profile_picture.'" /><h3 class="captain-text">'.$user.'</h3><span class="user-points">'.$l->total_points_formatted.'</span>'."<br/><p class='captain-text'>$title of $display</p>";
