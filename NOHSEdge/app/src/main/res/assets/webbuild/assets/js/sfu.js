...
//homescreen
var homescreenBGImg = ['assets/img/custom/homescreen.background.568.png'];
var homescreenBGVideo = []; //'assets/img/custom/images/custom/depaul-zone.mp4'
var homescreenModulesImg = 'assets/img/custom/homescreen.modules.568.png';
var homescreenSponsorImg = 'assets/img/custom/homescreen.sponsor.568.png';
...
$(document).ready( function(){
    ...
    rooturl = document.URL;
        ...
        rooturl.pop();
        rooturl = rooturl.join('/')+'/assets/scripts/';
    //facebook
    fbPost = {};
    fbPosting = false;
    fbReady = false;
...
refreshNOHSStudentID();
	...
	$('body').on('pageshow','#login',function(){
		if(loggedIn == true) {
			...
			//deleteCookie('UUID');
			$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'logout.php?nid='+nid,platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")});
			loggedIn = false;
	...
	FB.Event.subscribe('auth.authResponseChange', function(response) {
		if (response.status === 'connected') {
			$('img.facebook-login-logout-btn').attr('src', 'assets/img/fb-settings-logout-btn.png');
			...
			$('img.facebook-login-logout-btn').attr('src', 'assets/img/fb-settings-login-btn.png');
		}
	});
	$('body').on('click', '#login-btn',  function() {
		...
		$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'new_session.php?num=64'}, function(data) {
			console.log(data);
			if(getVar('uuid')==undefined) {uuid = data;} else {uuid = getVar('uuid');}
			setCookie('UUID',uuid,365);
			$.post(api_prefix+'assets/scripts/login.php', {url: http+'login.php?nid='+nid, user: user, pass: pass, platform: platform, uuid: getCookie("UUID"), version: platformVersion, app_version: appVersion, architecture: ostype, model: phoneModel, osname: platform, ostype: "32bit", is_jailbroken: isJailbroken}, function(data) {
				console.log(data);
	...
	$('#forgot-password-submit-btn').click(function() {
		...
		$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'forgot_password.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), email: $('#forgotPasswordTF').val()},  function(data) {
			var json = jQuery.parseJSON(data);
			if(json.response=='ok') {
				alert(json.msg);
	...
	$('body').on('click','#register-btn', function() {
		...
		$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'new_session.php?num=64'}, function(data) {
			...
			if(getVar('uuid')==undefined) {uuid = data} else {uuid = getVar('uuid')};
			setCookie('UUID',uuid,365);
			$.post(api_prefix+'assets/scripts/login.php', {url: http+'register.php?nid='+nid, email:email, user: user, pass: pass, platform: platform, uuid: getCookie("UUID"), version: platformVersion, app_version: appVersion, architecture: ostype, model: phoneModel, osname: platform, ostype: "32bit", is_jailbroken: isJailbroken}, function(data) {
				try {
					var json = jQuery.parseJSON( data );
				} catch(e) {
...
*/
	...
	$('body').on('click','#notification-view', function() {
	    ...
	    $.post(api_prefix+'assets/scripts/read-notifications.php', {url: http+'read_notifications.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
	    	console.log('read notification response: '+data);
	    });
    });
	$('body').on('click','.notification-fanpoll', function() {
		var pid = $(this).attr('data-pollId');
		$.post(api_prefix+'assets/scripts/fanpoll.php', {url: http+'get_poll_from_id.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), pid: pid},  function(data) {
    ...
    $('body').on('click','#homescreen-touch-target', function() {
	    ...
	    if( home_touch_target_action !== null ) {
		    ...
		    if( act[0] == 'url' ) {
				...
				$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'link_out.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), evurl: act[1], eid: 0}, function(data) {
					//console.log( data );
				});
				if(platform == 'Android') {
    ...
    $('body').on('click','div.heart-input',function(e) {
		...
		$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'heart.php?nid=' + nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), hkey: 'event', hvalue: eventid}, function(data) {
...
// 				alert('webcal://api.superfanu.com/calendar/ical.php?type=ics&token=' + token);
		...
		if(navigator.geolocation) {
			...
			var watchGeo = navigator.geolocation.watchPosition(function(position) {
				if(position.coords.accuracy <= 500 || count==4) {
					...
					$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'check_in.php?nid='+nid, eventid: eventid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), latitude: position.coords.latitude, longitude: position.coords.longitude, accuracy: position.coords.accuracy}, function(data) {
						var rMsg = $.parseJSON(data);
						console.log(rMsg);
	...
	$('body').on('click','tr.voteable', function() {
		$.post(api_prefix+'assets/scripts/fanpoll-vote.php', {url: http+'poll_vote.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), pid: $(this).parent().parent().attr('data-pid'), answer: $(this).find('h4').html()}, function(data) {
			//alert(data);
			console.log(data);
			if(data=='ok') {
				refreshFanPoll();
    ...
    $('body').on('click','#update-profile-btn', function() {
		...
		$.post(api_prefix+'assets/scripts/update_profile.php', {url: http+'update_profile.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), formdata: formElements}, function(data) {
			console.log(data);
			try {
				var json = jQuery.parseJSON( data );
			} catch(e) {
	...
	function refreshFAQ() {
        $.post(api_prefix+'assets/scripts/faq.php', {url: http+'get_faq.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), tkthttp: tkthttp},  function(data) {
	...
	$('#promo-submit-btn').click(function() {
		...
		$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'promo-code.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), invite: $('#promoTF').val()},  function(data) {
	    	var json = jQuery.parseJSON(data);
		        if(json.response == 'ok') {
					alert(json.msg);
					$('#promoTF').val('');
	...
	$('#invite-submit-btn').click(function() {
		...
		$('#inviteTF').val('');
		console.log(users);
		$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'invite.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), invite: JSON.stringify(users)},  function(data) {
			console.log(data);
		});
    });
	...
	$('body').on('click', '#soundboard a', function(e) {
		...
		if(soundboardIsPlaying == false) {
			...
			$('#soundboard img').attr('src','assets/img/play-black@2x.png');
			audio.parent().parent().find('img').attr('src','assets/img/stop-black@2x.png');
		} else {
			//stop old one first
	...
	$('body').on('click','#student-tickets-buy-message-send',function(e) {
		...
		if(msg == '') {return;}
		var send = $(this);
		$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'student_tickets_chat_post.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), stid: send.attr('data-stid'), to_uid: send.attr('data-touid'), message: msg}, function(data) {
	...
	$('body').on('click','#delete-tickets',function(e) {
		...
		if(confirm('Are you sure you want to remove these tickets from the listings? There is no undo.')) {
			$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'student_tickets_remove.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), stid: stid }, function(data) {
				getMyTicketsAndChats();
				$.mobile.changePage( '#student-tickets' );
			});
		}
	...
	$('#scores-add-btn').click(function() {
		...
		$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'add_scores.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), eid: scoreEIDSelected, us_score: $('#scores-add-us-score').val(), opp_score: $('#scores-add-opp-score').val()}, function(data) {
	...
	$('body').on('click','.link-track', function() {
		...
		if( link.attr('data-url') != undefined ) {
			if( link.attr('data-url') != '' ) {
				//have the URL, just need to track it
				$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'link_out.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), evurl: link.attr('data-url'), eid: link.attr('data-eid')}, function(data) {
					//console.log( data );
				});
				if(platform == 'Android') {
...
function checkLogin() {
	...
	if(loggedIn == true) {
		...
		$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'check_login.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), version: platformVersion, app_version: appVersion, architecture: ostype, model: phoneModel, osname: platform, ostype: "32bit", is_jailbroken: isJailbroken}, function(data) {
...
function refreshEvents() {
	$('#events .ui-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading Events...</p></div>');
	$.post(api_prefix+'assets/scripts/events.php', {url: http+'get_events.php?nid='+nid+'&headers=month', platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
...
function getEventInfo(eventid) {
	...
	$('#event-info .ui-content').html('<div class="loading"><p><img src="assets/img/ajax-loader.gif" /><p>Loading Event Info...</p></div>');
	$.post(api_prefix+'assets/scripts/event-info.php', {url: http+'event_info.php?nid='+nid, eventid: eventid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
...
function getEventInfoUsers( eventid ) {
	$('#event-info-users .ui-content').html('<div class="loading"><p><img src="assets/img/ajax-loader.gif" /><p>Loading Event Users...</p></div>');
	$.post(api_prefix+'assets/scripts/event-info-users.php', {url: http+'get_event_checked_in_users.php?nid='+nid, eid: eventid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
...
function refreshRewards() {
	$('#awards .ui-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading Rewards...</p></div>');
	$.post(api_prefix+'assets/scripts/rewards.php', {url: http+'get_awards_and_prizes.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), tkthttp: tkthttp},  function(data) {
...
function redeemAward( cid ) {
	...
	var longitude = 0;
	var accuracy = 0;
	$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'redeem_prize.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), latitude: latitude, longitude: longitude, accuracy: accuracy, cid: cid}, function(data) {
...
function refreshPrizeStore() {
	$('#prize-store .ui-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading Prize Store...</p></div>');
	$.post(api_prefix+'assets/scripts/prize-store.php', {url: http+'get_prize_store.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
...
function getPrizeStoreRedeemableItems( pid ) {
	//return;
	$.post(api_prefix+'assets/scripts/prize-store-redeemable-items.php', {url: http+'redeem_prize_store_purchased_items.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), pid: pid},  function(data) {
...
function buyPrize(pid, _obj) {
	...
	$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'redeem_prize_store.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), pid: pid, prize_option: prize_option},  function(data) {
...
function prizeStoreRedeemItem(pid, prid, _obj) {
	...
	_obj.html('<img src="assets/img/ajax-loader.gif" />');
	$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'redeem_prize_store_redeem_item.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), prid: prid},  function(data) {
		var json = JSON.parse( data );
		if(json.response == 'ok') {
			alert( json.title + "\n" + json.msg );
			getPrizeStoreRedeemableItems( pid );
...
function refreshHistory() {
	$('#history .ui-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading History...</p></div>');
	$.post(api_prefix+'assets/scripts/history.php', {url: http+'get_point_history.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
...
function refreshOffers() {
	$('#offers .ui-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading Offers...</p></div>');
	$.post(api_prefix+'assets/scripts/offers.php', {url: http+'get_offers.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
...
function redeemOffer(oid) {
	...
	var longitude = 0;
	var accuracy = 0;
	$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'redeem_offer.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), latitude: latitude, longitude: longitude, accuracy: accuracy, oid: oid}, function(data) {
		console.log(data);
		try {
			var resp = $.parseJSON( data );
		} catch(e) {
...
function refreshSocialFeed() {
	$('#social-feed .ui-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading Social Feed...</p></div>');
	$.post(api_prefix+'assets/scripts/social-feed.php', {url: http+'social_feed.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
...
function refreshLeaderboard() {
	$('#leaderboard .ui-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading Leaderboard...</p></div>');
	$.post(api_prefix+'assets/scripts/leaderboard.php', {url: http+'get_leaders.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
...
function refreshFanCam() {
	$('#fancam #fancam-pics').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading FanCam...</p></div>');
	$.post(api_prefix+'assets/scripts/fancam.php', {url: http+'get_media.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
...
function refreshScores() {
	$('#scores .ui-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading Scores...</p></div>');
	$.post(api_prefix+'assets/scripts/scores.php', {url: http+'get_scores.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
...
function refreshProfile() {
	$('#profile .ui-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading Account...</p></div>');
	$.post(api_prefix+'assets/scripts/profile.php', {url: http+'get_profile.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
	...
	$('#profile-edit .ui-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading Profile...</p></div>');
	$.post(api_prefix+'assets/scripts/profile-edit.php', {url: http+'get_profile.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
...
function refreshNotifications() {
	$('#notifications .ui-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading Notifications...</p></div>');
	$.post(api_prefix+'assets/scripts/notifications.php', {url: http+'get_notifications.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
...
function checkNotifications() {
	$.post(api_prefix+'assets/scripts/check-notifications.php', {url: http+'get_notifications.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
...
function refreshFanPoll() {
	...
	$('#fanpoll .ui-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading Poll...</p></div>');
	$.post(api_prefix+'assets/scripts/fanpoll.php', {url: http+'get_poll.php?nid='+nid+'&n='+n, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
		$('#fanpoll .ui-content').html(data).find('ul').listview().listview('refresh');
	});
}
...
function refreshNews() {
	if(rssLink!='') {
		$('#news .ui-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading News...</p></div>');
		$.post(api_prefix+'assets/scripts/news-pics.php', {url: rssLink, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
...
function refreshRosters() {
	$('#rosters .ui-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading Rosters...</p></div>');
	$.post(api_prefix+'assets/scripts/rosters.php', {url: http+'rosters.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
...
function refreshVenueList() {
	$('#venue-list .ui-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading Venues...</p></div>');
	$.post('assets/scripts/venue-list.php', {url: http+'get_venues.php?nid='+nid, platform: platform, uuid: uuid, login_key: login_key},  function(data) {
...
function getDynamicText() {
	$('#dynamic-text .ui-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading...</p></div>');
	$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'get_dynamic_text.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
		//console.log(data);
		try {
			var json = JSON.parse( data );
		} catch(e) {
...
function getDynamicTextTabs(option) {
	$('#dynamic-text-tabs .ui-content #dtt-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading...</p></div>');
	$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'get_dynamic_text_tabs.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), option: option},  function(data) {
		try {
			var json = JSON.parse( data );
		} catch(e) {
...
function getDynamicTextTabsOptions() {
	$('#dynamic-text-tabs-options .ui-content #dtt-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading...</p></div>');
	$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'get_dynamic_text_tabs_options.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
		console.log(data);
		try {
			var json = JSON.parse( data );
		} catch(e) {
...
function refreshBeacons() {
	if(platform=='Android') {
		$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'beacons.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
			try {
				var json = jQuery.parseJSON( data );
			} catch(e) {
				console.log(e);
...
function showIndividualPushNotfication( mid ) {
	...
	$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'get_notification.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), mid: mid}, function(data) {
		try {
			var json = jQuery.parseJSON( data );
		} catch(e) {
		...
		for (var not in json) {
			if( json.hasOwnProperty( not ) ) {
				...
				$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'read_notification.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), id: json[not].id}, function(data) {
					//nothing
				});
...
function getMyTicketsAndChats() {
	$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'student_tickets_mine.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")}, function(data) {
		var json = jQuery.parseJSON( data );
		var myTickets = '';
...
function getTaggedTicketEvents() {
	$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'student_tickets_events.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")}, function(data) {
		var json = jQuery.parseJSON( data );
		if(json.count > 0) {
...
function getAvailableTickets() {
	...
	$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'student_tickets_buy.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), eid: $('#student-tickets-buy-eid').val()}, function(data) {
		var json = jQuery.parseJSON( data );
		var tickets = '';
		if(json.response == 'ok') {
			if(json.count > 0) {
...
function getStudentTicketsChat(stid, threadid, touid) {
	...
	$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'student_tickets_chat.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), stid: stid, threadid: threadid}, function(data) {
		var json = jQuery.parseJSON( data );
		if(json.response == 'ok') {
			$('#student-tickets-chat-chat').html('');
			var chat = '';
...
function getTicketChats(stid) {
	$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'student_tickets_chats.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), stid: stid}, function(data) {
		var json = jQuery.parseJSON( data );
		var chats = '';
		if(json.count > 0) {
...
function refreshNOHSStudentID() {
    $('#nohs-student-id .ui-content').html('<div class="loading"><img src="assets/img/ajax-loader.gif" /><p>Loading Student ID...</p></div>');
	$.post(api_prefix+'assets/scripts/nohs_student_id.php', {url: http+'nohs_sports_pass.php?nid='+nid, platform: platform, uuid: uuid, login_key: login_key},  function(data) {
...
function checkForNewHomescreen() {
	...
	$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'check_for_homescreen_updates.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key")},  function(data) {
		try {
			var json = jQuery.parseJSON( data );
		} catch(e) {
...
function checkForAds(pageKey, pageHash) {
	...
	$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'get_page_ad.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), pageKey: pageKey},  function(data) {
		try {
			var json = jQuery.parseJSON( data );
		} catch(e) {
			return false;
...
function androidLoginViaFacebook(uuid, response, platformVersion, phoneModel, isJailbroken) {
	$.post(api_prefix+'assets/scripts/fb-login.php', {url: http+'fb_login.php?nid='+nid, platform: 'Android', uuid: getCookie("UUID"), fbdata: response, version: platformVersion, app_version: appVersion, architecture: '32bit', model: phoneModel, osname: platform, ostype: "32bit", is_jailbroken: isJailbroken}, function(data) {
		try {
		    console.log(data);
...
function loginViaFacebook() {
	$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'new_session.php?num=64'}, function(data) {
		...
		FB.api('/me?fields=id,name,first_name,last_name,email,age_range,link,gender,locale,timezone,updated_time,verified', function(response) {
			...
			$.post(api_prefix+'assets/scripts/fb-login.php', {url: http+'fb_login.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), fbdata: response, version: platformVersion, app_version: appVersion, architecture: ostype, model: phoneModel, osname: platform, ostype: "32bit", is_jailbroken: isJailbroken}, function(data) {
				try {
					var rr = jQuery.parseJSON( data );
				} catch(e) {
					alert('We are not able to log you in at this time. Please try again in a moment.');
					...
					FB.ui(postdata,
						function(response) {
						    if (response && response.post_id) {
						    	$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'fb_post.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), eventid: fbPost.eventid}, function(data) {
						    		//shhhhh
						    	});
						    	fbLast.action = null;
	...
	twttr.ready(
		function (twttr) {
			twttr.events.bind('tweet',function (event) {
				$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'twitter_post.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), eid: twPost.eid}, function(data) {
	    			//shhhhh
	    			if(data == 'ok') {
			...
			twttr.events.bind('retweet',function (event) {
				$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'social_interaction.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), source: 'social_feed', social_network: 'twitter', social_uuid: '', action: 'retweet'}, function(data) {
		...
		if(posted == "true") {
			if(action == 'tweet') {
				if( getVarFromURL('in_reply_to', url) != undefined) {
					$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'social_interaction.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), source: 'social_feed', social_network: 'twitter', social_uuid: getVarFromURL('in_reply_to', url), action: 'reply'}, function(data) {
			    		if(data == 'ok') {
			    			alert('Your tweet was sent successfully!');
			    			//we should just update the image
							//$('#social-feed li.sf-'+social_uuid+' .actions img.twitter-retweet').attr('src', 'assets/img/social/twitter/retweet_on.png');
						} else {
			    			alert('There was an error tweeting that');
						}
			    	});
				} else {
					$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'twitter_post.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), eid: twPost.eid, return_url: url}, function(data) {
			    		if(data == 'ok') {
			    			alert('Your message was posted to Twitter!');
						} else {
			    			alert('There was an error posting to Twitter');
			...
			if(action == 'retweet') {
				...
				$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'social_interaction.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), source: 'social_feed', social_network: 'twitter', social_uuid: social_uuid, action: action}, function(data) {
		    		if(data == 'ok') {
				...
				if( getVarFromURL('already_favorited', url) == 'false') {
					$.post(api_prefix+'assets/scripts/ajax.php', {url: http+'social_interaction.php?nid='+nid, platform: platform, uuid: getCookie("UUID"), login_key: getCookie("login_key"), source: 'social_feed', social_network: 'twitter', social_uuid: social_uuid, action: action}, function(data) {
			    		if(data == 'ok') {
			    			...
			    			//we should just update the image
							$('#social-feed li.sf-'+social_uuid+' .actions img.twitter-favorite').attr('src', 'assets/img/social/twitter/favorite_on.png');
						} else {
