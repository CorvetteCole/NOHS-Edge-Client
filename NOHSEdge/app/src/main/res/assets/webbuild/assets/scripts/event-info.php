...
**/
	...
	if($ev->checkedin == '') {
			...
			if($ev->publicTags) {
				foreach($ev->publicTags as $publicTag) {
					...
					if($user && $title && $profile_picture && $display) {
					    echo "<div class='captain-item'><img class='user-pic-crown' src='assets/img/custom/captain-crown.png'><img class='user-pic' src='$profile_picture'/><h2>$user</h2><h3>$title of $display</h3></div>";
					}
				}
			}
		...
		if($ev->checkintime) {
			...
			//twitter row
			$tw = $ev->twitter;
			echo '<div class="event-info-link social-twitter" data-message="'.addslashes($tw->tweetString).'" data-picture="'.addslashes($tw->picture).'" data-eid="'.$_POST['eventid'].'"><img src="assets/img/twitter-icon.png" /><p><a href="https://twitter.com/intent/tweet?text='.urlencode($tw->tweetString).'&url='.urlencode($tw->picture).'">Share on Twitter</a></p></div>';
			//facebook row
			$fb = $ev->facebook;
			echo '<div class="event-info-link social-facebook" data-link="'.$fb->link.'" data-name="'.addslashes($fb->name).'" data-message="'.addslashes($fb->message).'" data-caption="'.addslashes($fb->caption).'" data-picture="'.addslashes($fb->picture).'" data-description="'.addslashes($fb->description).'" data-eid="'.$_POST['eventid'].'"><img src="assets/img/facebook-icon.png" /><p>Share on Facebook</p></div>';
